...
**Created**: 2026-02-09  
**Author**: 鸡哥 (Keith's AI Assistant)  
...
|--------|-------------|----------|
| **Simple Sync** | Upload file → GitHub CDN → Notion link | Single files (papers, reports) |
| **Report Sync** | Markdown → Main page + Subpages | Multi-chapter documents |
...
**Env loading note**:
- Clawdbot loads env vars from (in order): current working directory `.env`, global `~/.clawdbot/.env` (aka `~/.openclaw/.env`), then config `env` block.
- Recommended: put long‑lived keys in `~/.openclaw/.env` and keep project‑specific overrides in the workspace `.env`.
...
### Resources Database (主数据库)

| Field | Type | Description |
|-------|------|-------------|
| Recurso | title | File title |
| URL | url | CDN link |
| 类型 | multi_select | File type (PDF, MD, Image) |
| 标签 | multi_select | Tags (Auto-Sync, CDN, etc.) |
...
data = {
    ...
    "properties": {
        "Recurso": {"title": [{"text": {"content": "文件名.md"}}]},
        "URL": {"url": "https://cdn.jsdelivr.net/..."},
        "类型": {"multi_select": [{"name": "MD"}, {"name": "Research"}]},
...
```
Local File → GitHub (CDN) → Notion Database
                        ↓
                  Main Page (CDN Link)
                        ↓
...
└──────┬──────┘
       ↓
┌─────────────┐
│  GitHub API  │  ← Upload + Base64 encode
└──────┬──────┘
       ↓
┌─────────────┐     ┌─────────────┐
│ jsDelivr CDN │────→│  CDN Link   │
└─────────────┘     └──────┬──────┘
                           ↓
                   ┌─────────────┐
                   │  Notion API │  ← Create page with CDN link
                   └──────┬──────┘
                          ↓
                   ┌─────────────┐
                   │ Notion Page │
...
def create_page_with_subpages(title, filepath, database_id):
    ...
    main_page_data = {
        ...
        "properties": {
            "Recurso": {"title": [{"text": {"content": f"🦞 {title}"}}]},
            "URL": {"url": get_cdn_url(filepath)},
            "类型": {"multi_select": [{"name": "调研报告"}]},
            "标签": {"multi_select": [{"name": "Auto-Sync"}]}
        },
        "children": [
            {
                "object": "block",
                "type": "paragraph",
                "paragraph": {"rich_text": [{"type": "text", "text": {"content": f"📑 本报告共{len(chapters)}章节"}}]}
        ]
    }
    ...
    chapter_names = ["SCQA与概述", "公司概况", "核心产品", "市场定位"]
    max_subpages = 4
    ...
    for i in range(max_subpages):
        ...
        subpage_data = {
            ...
            "properties": {"title": [{"text": {"content": f"第{start+1}-{end}章: {chapter_names[i]}"}}]},
            "children": blocks
        }
...
```bash
cd /Users/tianyunkeji/clawd/市场调研/卫星通讯地面段服务商
python3 sync_to_notion_subpages.py
```
